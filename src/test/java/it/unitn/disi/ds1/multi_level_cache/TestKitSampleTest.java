...
public class TestKitSampleTest {
    ...
    static ActorEnvironment environment;
    static ActorSystem system;

    @BeforeClass
    public static void setup() {
        environment = new ActorEnvironment("TestSystem", 2, 4, 1);
        system = environment.getActorSystem();
    }
    ...
    public void testIt() {
        ...
        new TestKit(system) {
            {
                final ActorRef clientOne = environment.getClient(0).orElseThrow();
                final ActorRef firstL1 = environment.getL1Cache(0).orElseThrow();
                final TestKit probe = new TestKit(system);
                probe.watch(clientOne);

                environment.makeClientRead(clientOne, firstL1, 3);
                probe.expectMsgClass(ReadReplyMessage.class);
