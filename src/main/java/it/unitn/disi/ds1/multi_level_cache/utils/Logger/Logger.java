...
public final class Logger {
    ...
    private final static String CRITICAL_WRITE_ABORT_FORMAT = "uuid: %s, key: %d";
    private final static String CRITICAL_WRITE_COMMIT_FORMAT_REC = "uuid: %s, key: %d, new-value: %d, old-value: %d, new-uc: %d, old-uc: %d";
    private final static String CRITICAL_WRITE_COMMIT_FORMAT_SEND = "uuid: %s, key: %d, value: %d, uc: %d";
    private final static String CRITICAL_WRITE_REQUEST_REC = "uuid: %s, key: %d";
    private final static String CRITICAL_WRITE_REQUEST_SEND = "uuid: %s, key: %d";
    private final static String CRITICAL_WRITE_VOTE = "uuid: %s, key: %d, is-ok: %b";
    ...
    public static void criticalWrite(String id, UUID uuid, LoggerOperationType operationType, int key, int value, boolean isLocked) {
        String msg = "";

        if (operationType == LoggerOperationType.RECEIVED) {
            msg = String.format(WRITE_FORMAT_REC, uuidToString(uuid), key, value, isLocked);
        } else if (isSendAction(operationType)) {
            msg = String.format(WRITE_FORMAT_SEND, uuidToString(uuid), key, value);
        }

        log(MessageType.CRITICAL_WRITE, id, operationType, msg);
    }

    public static void criticalWriteAbort(String id, UUID uuid, LoggerOperationType operationType, int key) {
        String msg = String.format(CRITICAL_WRITE_ABORT_FORMAT, uuidToString(uuid), key);
        log(MessageType.CRITICAL_WRITE_ABORT, id, operationType, msg);
    }

    public static void criticalWriteCommit(String id, UUID uuid, LoggerOperationType operationType, int key, int newValue, int oldValue, int newUc, int oldUc) {
        String msg = "";

        if (operationType == LoggerOperationType.RECEIVED) {
            msg = String.format(CRITICAL_WRITE_COMMIT_FORMAT_REC, uuidToString(uuid), key, newValue, oldValue, newUc, oldUc);
        } else if (isSendAction(operationType)) {
            msg = String.format(CRITICAL_WRITE_COMMIT_FORMAT_SEND, uuidToString(uuid), key, newValue, newUc);
        }

        log(MessageType.CRITICAL_WRITE_COMMIT, id, operationType, msg);
    }

    public static void criticalWriteRequest(String id, UUID uuid, LoggerOperationType operationType, int key, boolean isOk) {
        String msg = "";

        if (operationType == LoggerOperationType.RECEIVED) {
            msg = String.format(CRITICAL_WRITE_REQUEST_REC, uuidToString(uuid), key, isOk);
        } else if (isSendAction(operationType)) {
            msg = String.format(CRITICAL_WRITE_REQUEST_SEND, uuidToString(uuid), key);
        }

        log(MessageType.CRITICAL_WRITE_REQUEST, id, operationType, msg);
    }

    public static void criticalWriteVote(String id, UUID uuid, LoggerOperationType operationType, int key, boolean isOk) {
        String msg = String.format(CRITICAL_WRITE_VOTE, uuidToString(uuid), key, isOk);
